function A_all = alignAll(raws, ref)
    ...
    for i = 1:n_frames
        if i ~= ref
            ...
            % level 4:
            l4align = L1Align(ds4(ref,:,:), ds4(i,:,:), no_align, tile_sizes(4), search_radius(4)); 
            ...
            % level 3:
            l3align = L1Align(ds3(ref,:,:), ds3(i,:,:), l4align, tile_sizes(3), search_radius(3));
            % map up l3align 
            l3align = repelem(l3align, tile_sizes(3) * ds_factors(2), tile_sizes(3) * ds_factors(2),1)*ds_factors(2);

            % level 2:
            l2align = L1Align(ds2(ref,:,:), ds2(i,:,:), l3align, tile_sizes(2), search_radius(2));
            % map up l2align 
